...
from sharedDB.database import Database
from appMain.userDB.Client import Client
import csv
import json
import os
import pandas as pd
...
def redirect_home_temp(message=None):
    r1 = open('C:/Users/Larry/Desktop/MYPilotProject/logicpart/user.csv')  # Here your csv file
    r = csv.reader(r1)
    lines = list(r)
    lines[0][0] = tempvalue
    writer1 = open('C:/Users/Larry/Desktop/MYPilotProject/logicpart/user.csv', 'w', newline='')
    writer = csv.writer(writer1)
    writer.writerows(lines)
    writer1.close()
    r1.close()
    r1 = open('C:/Users/Larry/Desktop/MYPilotProject/logicpart/user.csv')
    fout = open("C:/Users/Larry/Desktop/MYPilotProject/logicpart/keystroke.trial5.csv", "a")
    for line in r1:
        fout.write(line)
    fout.close()
    ...
    path = "C:/Users/Larry/Desktop/MYPilotProject/logicpart/keystroke.trial.csv"
...
def redirect_reg_captcha(message=None):
    ...
    writer1 = open('C:/Users/Larry/Desktop/MYPilotProject/logicpart/user.csv', 'w')
    writer = csv.writer(writer1)
    writer.writerows(lines)
    writer1.close()
    r1.close()
    r1 = open('C:/Users/Larry/Desktop/MYPilotProject/logicpart/user.csv')
    fout = open("C:/Users/Larry/Desktop/MYPilotProject/logicpart/keystroke.trial.csv", "a")
    for line in r1:
        fout.write(line)
    fout.close()
    return render_template('index.html', message=message if message is not None else None)
...
def redirect_client_account_summary(message=None):
    clients = Client.from_email(session['email'])
    print(clients)
    return render_template('client_accountsummary.html', message=message if message is not None else None, clients=clients)
...
def login_client():
    ...
    if client is not None:
        ...
        #return render_template('index.html', target="blank")
        session['email'] = None
    return render_template('index.html', target="blank")
...
def emp_add():
    if request.method == 'GET' and session['email'] != None:
        ...
        tempvalue_reg = name
        password = request.form.get('Client_pass')
        Gender = request.form.get('Client_gender')
        Contact_number = request.form.get('Client_pno')
        _id = request.form.get('Client_ID')
        Date_of_Birth = request.form.get('Client_DoB')
        email = request.form.get('Client_mail')
        clients2 = Client(_id, password, name, Contact_number,Gender, email, Date_of_Birth)
        clients2.save_to_mongo()
        # message = "Successful"
        return render_template('reg_captcha.html', message="success")
...
def delete_client():
    name = request.form.get['Client_name']
    account = request.form.get['Client_account']
    pno = request.form.get['Client_pno']

    clients = Client.from_id(name)
    clients.delete_client(name)
    # print(User.login_vaild(email,password))
    if clients is not None:
        if clients.account_number == account:
